# Data-Engineering Plugin Analysis — May 21, 2026

This is a comparison of two versions of the plugin that the Studio app uses to automate its data work. The plugin is a bundle of automated tasks the app calls out to: figure out what a user wants, pull data in (ingestion with dlt), and shape that data (transformation with dbt).
...
- [`01-old-version.md`](./01-old-version.md) — What the older plugin contains: its automated jobs, helper scripts, and built-in startup behavior.
- [`02-new-version.md`](./02-new-version.md) — Same inventory for the newer plugin, plus the new step-by-step plan file that drives it.
...
The newer plugin is a rewrite of *how the work flows*, not a rewrite of what it can do:

1. **The auto-start helper was removed.** The old plugin had a small script that ran every time a session began. It pasted in classification instructions and the domain configuration so the assistant always knew where to put things. That script is gone.
...
5. **The plugin is now packaged as one of a family.** The name gained a `-med` suffix (for "medium") and the version was reset, hinting at "low" and "high" siblings that share the same automated jobs but offer different depth of guidance.
